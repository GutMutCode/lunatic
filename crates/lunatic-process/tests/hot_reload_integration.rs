...
async fn test_basic_memory_snapshot_and_restore() -> Result<()> {
    ...
    let increment = instance_v1.get_typed_func::<(), i32>(&mut store, "increment")?;
    let get_count = instance_v1.get_typed_func::<(), i32>(&mut store, "get_count")?;
    ...
    let initial_value = i32::from_le_bytes([
        initial_bytes[0],
        initial_bytes[1],
        initial_bytes[2],
        initial_bytes[3],
    ...
    let value_after = i32::from_le_bytes([
        bytes_after[0],
        bytes_after[1],
        bytes_after[2],
        bytes_after[3],
    ...
    assert_eq!(
        count_after_reload, 3,
        "Counter should still be 3 after reload"
    );
    ...
    let reset_v2 = instance_v2.get_typed_func::<(), ()>(&mut store_v2, "reset")?;
