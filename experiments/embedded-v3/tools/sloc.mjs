...
// Frozen embedded-v3 SLOC/accounting tool. Node.js 24, no third-party packages.
...
function classifyExternalPath(candidate, candidateRoot, resolvedPath) {
  ...
  if (candidate === "extism") {
    if (isWithin(REPOSITORY_ROOT, resolvedPath)) {
      throw new Error(`extism may not depend on a Lunatic repository path: ${resolvedPath}`);
    }
...
function selfTest() {
  ...
  const results = { normal_fixture: "pending", missing_attribution: "pending", symlink_reparse: "pending" };
  try {
