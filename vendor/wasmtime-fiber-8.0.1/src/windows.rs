    ...
    pub(crate) fn resume<A, B, C>(&self, _stack: &FiberStack, result: &Cell<RunResult<A, B, C>>) {
        unsafe {
            ...
            let parent_fiber = if is_fiber {
                ...
                // Rust 1.95 uses Fiber Local Storage to register an internal hook that
                // runs thread-local destructors. Register the hook on the original thread
                // before converting it into a fiber so deleting a child Wasmtime fiber
                // cannot run the thread-exit hook on the wrong fiber.
                //
                // Backported from bytecodealliance/wasmtime#12426 (aedc54800061).
                struct Guard;
