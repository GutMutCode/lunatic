...
이 문서는 Lunatic 런타임에 Hot Code Reloading 기능을 추가하기 위한 아키텍처 제안입니다. Erlang/BEAM의 hot code loading 메커니즘에서 영감을 받았으며, WebAssembly의 특성과 Lunatic의 현재 아키텍처를 고려하여 설계되었습니다.
...
- `WasmProcess`: 실행 중인 WASM 모듈 인스턴스
- Signal 기반 프로세스 간 통신
- Process ID와 Signal mailbox로 식별

**Environment (`lunatic-process/src/env.rs`)**
- `LunaticEnvironment`: 프로세스들을 관리하는 환경
- `DashMap<u64, Arc<dyn Process>>`로 프로세스 저장
- 프로세스 생성/제거/조회 기능

**Module Compilation (`lunatic-process/src/runtimes/wasmtime.rs`)**
- `WasmtimeRuntime`: WASM 모듈 컴파일 엔진
- `WasmtimeCompiledModule`: 컴파일된 모듈 (Arc로 공유)
- `compile_module()`: 모듈 컴파일
- `instantiate()`: 인스턴스 생성
